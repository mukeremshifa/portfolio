...
| ~~Project `links.*` hosts~~ | **Closed 2026-09-04.** All seven projects' `github.com/mukeremshifa/…` and `<slug>.mukeremshifa.com` URLs | **Verified by hand by the owner**, every one opened. The survey project's repository was renamed on GitHub to match its slug's spelling and the URL corrected here the same day (`gamified-servey-project` → `gamified-survey-project`). Nothing automated guards these — `tests/unit/links.test.ts` was deleted by `b4d5d05` and is recoverable at `b4d5d05^` |
| ~~`cover.src` — the other three~~ | **Closed 2026-09-04.** `amteclinks-website-seo`, `java-shopping-cart` and `little-lemon-capstone` have **no `cover`, deliberately** | **Nothing.** The owner's decision: none of the three has a front-end build worth covering, so a render would misrepresent them. `cover` became optional on 2026-08-31 and the card falls back, so this renders correctly today. This row is struck rather than deleted because the absence is invisible in the UI — the inventory is the only place it reads as a choice rather than an oversight, which is what this row was opened to prevent. Revisit if any of the three gains a front end |
