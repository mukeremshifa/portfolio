 ...
 * photograph and the difference matters here: `portrait` is 4:5 for the hero, and putting
 * a 4:5 image behind `rounded-full` crops the top of a head off. Absent `avatar`, this
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <dl className="flex flex-col gap-4 border-t border-border-subtle pt-6 sm:grid sm:grid-cols-[max-content_1fr] sm:items-baseline sm:gap-x-8 sm:gap-y-3">
        {/* Location only. `location.remote` drives the footer's "Available remotely", which
            is a claim about working arrangements — this row answers "where are they", and
            mixing the two makes one line say two things that change independently. */}
